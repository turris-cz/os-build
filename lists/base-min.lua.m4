...
-- Turris utility
Install("turris-utils", "user-notify", "oneshot", "watchdog_adjust", { priority = 40 })
if for_l10n then
...
elseif model:match("[Oo]mnia") then
	Install("rainbow-omnia", "sfpswitch", "libatsha204", "update_mac", { priority = 40 })
elseif model:match("^[Tt]urris$") then
	Install("rainbow", "libatsha204", "update_mac", { priority = 40 })
end
